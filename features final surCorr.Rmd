...
{
  index <- which(x$surface == "None" | x$surface == "")
  if(length(index) != 0)
  {
    x <- x[-index,]
  }
...
# Remove records played on carpet
Too few records played on this surface
```{r}
clean.carpet <- function(x)
{
  index <- which(x$surface == "Carpet")
  if(length(index) != 0)
  {
    x <- x[-index,]
  }
...
atplist <- lapply(atplist, clean.carpet)
```
...
# Function to calculate features by surface
Uses training.hard, training.clay, and training.grass global datasets created from the training data.

metric.w is a string for the relevant variable where the player is the winner of the match.
metric.l is a string for the same variable where the player is the loser of the match.
```{r}

calc.surface <- function(player_id, metric.w, metric.l, surface) 
{
  colw.index <- which(names(training_data_temp) == metric.w)
  if(length(colw.index) == 0) #this should never happen
  {
    browser()
    stop("Error in surface function: incorrect metric provided")
  }
  coll.index <- which(names(training_data_temp) == metric.l)
  if(length(coll.index) == 0) #this should never happen
  {
    browser()
    stop("Error in surface function: incorrect metric provided")
  }
  ...
  # all matches on hard courts
  tempw.hard <- training.hard[which(training.hard$winner_id == player_id),]
  templ.hard <- training.hard[which(training.hard$loser_id == player_id),]
  # all matches on clay courts
  tempw.clay <- training.clay[which(training.clay$winner_id == player_id),]
  templ.clay <- training.clay[which(training.clay$loser_id == player_id),]
  # all matches on carpet courts
  #tempw.carpet <- training.hard[which(training.carpet$winner_id == player_id),]
  #templ.carpet <- training.hard[which(training.carpet$loser_id == player_id),]
  # all matches on grass courts
  tempw.grass <- training.grass[which(training.grass$winner_id == player_id),]
  templ.grass <- training.grass[which(training.grass$loser_id == player_id),]
  ...
  # for matches on hard
  if(nrow(tempw.hard) == 0 & nrow(templ.hard) == 0)
  {
    n.hard = -1 # player didn't play any matches on hard
  } 
  else if(nrow(tempw.hard) == 0) # player never won a match on hard 
  {
    t3.hard <- mean(templ.hard[,metric.l])
  }
  else if(nrow(templ.hard) == 0) # player never lost a match on hard
  {
    t3.hard <- mean(tempw.hard[,metric.w])
  }
  else # player both won and lost matches on hard
  {
    t1.hard <- mean(templ.hard[,metric.l])
    t2.hard <- mean(tempw.hard[,metric.w])
    t3.hard <- mean(c(t1.hard, t2.hard))
  }
  
  
  # for matches on clay
  if(nrow(tempw.clay) == 0 & nrow(templ.clay) == 0)
  {
    n.clay = -1 # player didn't play any matches on clay
  } 
  else if(nrow(tempw.clay) == 0) # player never won a match on clay 
  {
    t3.clay <- mean(templ.clay[,metric.l])
  }
  else if(nrow(templ.clay) == 0) # player never lost a match on clay
  {
    t3.clay <- mean(tempw.clay[,metric.w])
  }
  else # player both won and lost matches on clay
  {
    t1.clay <- mean(templ.clay[,metric.l])
    t2.clay <- mean(tempw.clay[,metric.w])
    t3.clay <- mean(c(t1.clay, t2.clay))
  }
  
  
  # for matches on carpet
  # if(nrow(tempw.carpet) == 0 & nrow(templ.carpet) == 0)
  # {
  #   n.carpet = -1 # player didn't play any matches on carpet
  # } 
  # else if(nrow(tempw.carpet) == 0) # player never won a match on carpet 
  # {
  #   t3.carpet <- mean(templ.carpet[,metric.l])
  # }
  # else if(nrow(templ.carpet) == 0) # player never lost a match on carpet
  # {
  #   t3.carpet <- mean(tempw.carpet[,metric.w])
  # }
  # else # player both won and lost matches on carpet
  # {
  #   t1.carpet <- mean(templ.carpet[,metric.l])
  #   t2.carpet <- mean(tempw.carpet[,metric.w])
  #   t3.carpet <- mean(c(t1.carpet, t2.carpet))
  # }
  
  
  # for matches on grass
  if(nrow(tempw.grass) == 0 & nrow(templ.grass) == 0)
  {
    n.grass = -1 # player didn't play any matches on grass
  } 
  else if(nrow(tempw.grass) == 0) # player never won a match on grass 
  {
    t3.grass <- mean(templ.grass[,metric.l])
  }
  else if(nrow(templ.grass) == 0) # player never lost a match on grass
  {
    t3.grass <- mean(tempw.grass[,metric.w])
  }
  else # player both won and lost matches on grass
  {
    t1.grass <- mean(templ.grass[,metric.l])
    t2.grass <- mean(tempw.grass[,metric.w])
    t3.grass <- mean(c(t1.grass, t2.grass))
  }
  
  
  weighted.sum <- 0
  #if(is.na(surface) | is.null(surface))
  #{
  #  browser()
  #}
  ...
  if(surface == "Hard")
  {
    weighted.sum <- t3.hard + 0.4089*t3.clay + 0.5359*t3.grass
  }
  else if(surface == "Clay")
  {
    weighted.sum <- 0.4089*t3.hard + t3.clay + 0.3451*t3.grass
  }
  # else if(surface == "Carpet")
  # {
  #   weighted.sum <- 0.35*t3.hard + 0.31*t3.clay + t3.carpet + 0.25*t3.grass
  # }
  else if(surface == "Grass")
  {
    weighted.sum <- 0.5359*t3.hard + 0.3451*t3.clay + t3.grass
  }
  else # this should never happen
  {
    browser()
    stop("Surface not recognized")
  }
  
  
  # check that player does in fact have recorded matches
  if(n.hard + n.clay + n.grass == -3)
  {
    browser()
    stop("Error in surface function: player has no recorded matches on any surface") 
  }
  
  t3 <- weighted.sum/(3 + n.hard + n.clay + n.grass) # weighted mean
  return(t3)
}
```
...
calc_1stReturn_rally <- function(dataset, player_id)
{
  ...
  {
    t3 <- 1 - mean(temp2$w_1stRally)
  }
  else if(nrow(temp2) == 0) #if player never lost a match
  {
    t3 <- 1 - mean(temp1$l_1stRally)
  }
  else
  {
    t1 <- 1 - mean(temp1$l_1stRally)
    t2 <- 1 - mean(temp2$w_1stRally)
    t3 <- mean(c(t1,t2)) #percentage of points won on serve for player i
  }
...
calc_2ndReturn_rally <- function(dataset, player_id)
{
  ...
  {
    t3 <- 1 - mean(temp2$w_2ndRally)
  }
  else if(nrow(temp2) == 0) #if player never lost a match
  {
    t3 <- 1 - mean(temp1$l_2ndRally)
  }
  else
  {
    t1 <- mean(temp1$w_1stIn_prob)
    t2 <- mean(temp2$l_1stIn_prob)
    t3 <- mean(c(f1,f2)) #percentage of points won on serve for player i
  }
...
{
  new <- x[,-which(names(x) %in% c("tourney_id", "surface","tourney_name", "tourney_level", "tourney_date", "match_num", "winner_id", "loser_id", "winner_name","loser_name", "w_total_2nd", "l_total_2nd", "w_2ndLost", "l_2ndLost", "w_1stWon_prob", "l_1stWon_prob", "w_2ndWon_prob", "l_2ndWon_prob", "w_1stWon_givenIn_prob", "l_1stWon_givenIn_prob"))]
  return(new)
...
This is done to make it easier to apply the surface weightings. The surface weighting matrix is given by... MAKE TABLE.

```{r}
training_data_temp <- bind_rows(atplist[-1])
hard.index <- which(training_data_temp$surface == "Hard")
clay.index <- which(training_data_temp$surface == "Clay")
carpet.index <- which(training_data_temp$surface == "Carpet")
grass.index <- which(training_data_temp$surface == "Grass")

training.hard <- training_data_temp[hard.index,]
training.clay <- training_data_temp[clay.index,]
training.carpet <- training_data_temp[carpet.index,]
training.grass <- training_data_temp[grass.index,]
```
...
```{r}
calc_testing_data <- function(x) # x refers to testing data.
{
  ...
  {
     #temp <- x[i,]
     x$w_1stIn_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_1stIn_prob", metric.l = "l_1stIn_prob", surface = x$surface[i])
     x$l_1stIn_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_1stIn_prob", metric.l = "l_1stIn_prob", surface = x$surface[i])
     x$w_ace_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_ace_prob", metric.l = "l_ace_prob", surface = x$surface[i])
     x$l_ace_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_ace_prob", metric.l = "l_ace_prob", surface = x$surface[i])
     x$w_df_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_df_prob", metric.l = "l_df_prob", surface = x$surface[i])
     x$l_df_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_df_prob", metric.l = "l_df_prob", surface = x$surface[i])
     x$w_1stRally[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_1stRally", metric.l = "l_1stRally", surface = x$surface[i])
     x$l_1stRally[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_1stRally", metric.l = "l_1stRally", surface = x$surface[i])
     x$w_2ndRally[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_2ndRally", metric.l = "l_2ndRally", surface = x$surface[i])
     x$l_2ndRally[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_2ndRally", metric.l = "l_2ndRally", surface = x$surface[i])
     x$w_svptWon_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_svptWon_prob", metric.l = "l_svptWon_prob", surface = x$surface[i])
     x$l_svptWon_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_svptWon_prob", metric.l = "l_svptWon_prob", surface = x$surface[i])
     x$w_returnWon_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_returnWon_prob", metric.l = "l_returnWon_prob", surface = x$surface[i])
     x$l_returnWon_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_returnWon_prob", metric.l = "l_returnWon_prob", surface = x$surface[i])
     x$w_serveAdv[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_serveAdv", metric.l = "l_serveAdv", surface = x$surface[i])
     x$l_serveAdv[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_serveAdv", metric.l = "l_serveAdv", surface = x$surface[i])
     x$w_bpLost_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_bpLost_prob", metric.l = "l_bpLost_prob", surface = x$surface[i])
     x$l_bpLost_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_bpLost_prob", metric.l = "l_bpLost_prob", surface = x$surface[i])
     x$w_bpServe_freq[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_bpServe_freq", metric.l = "l_bpServe_freq", surface = x$surface[i])
     x$l_bpServe_freq[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_bpServe_freq", metric.l = "l_bpServe_freq", surface = x$surface[i])
     x$w_bpConvert_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_bpConvert_prob", metric.l = "l_bpConvert_prob", surface = x$surface[i])
     x$l_bpConvert_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_bpConvert_prob", metric.l = "l_bpConvert_prob", surface = x$surface[i])
     x$w_bpReceive_freq[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_bpReceive_freq", metric.l = "l_bpReceive_freq", surface = x$surface[i])
     x$l_bpReceive_freq[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_bpReceive_freq", metric.l = "l_bpReceive_freq", surface = x$surface[i])
     x$w_SvGmsWon_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_SvGmsWon_prob", metric.l = "l_SvGmsWon_prob", surface = x$surface[i])
     x$l_SvGmsWon_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_SvGmsWon_prob", metric.l = "l_SvGmsWon_prob", surface = x$surface[i])
     x$w_rtnGmsWon_prob[i] = calc.surface(player_id =  x$winner_id[i], metric.w = "w_rtnGmsWon_prob", metric.l = "l_rtnGmsWon_prob", surface = x$surface[i])
     x$l_rtnGmsWon_prob[i] = calc.surface(player_id =  x$loser_id[i], metric.w = "w_rtnGmsWon_prob", metric.l = "l_rtnGmsWon_prob", surface = x$surface[i])
     #training_data <- bind_rows(temp, training_data)
  }
...
#training_data_temp <- bind_rows(atplist[-1]) # line is now earlier

testing_data1 <- calc_testing_data(atplist$atp2019) 
...
```{r}
setwd(getwd())
write.csv(training_data,"./train_test_data/surfaceCorr_training.csv", row.names = FALSE)
write.csv(testing_data,"./train_test_data/surfaceCorr_testing.csv", row.names = FALSE)
```
