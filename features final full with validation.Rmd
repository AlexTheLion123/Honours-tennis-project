...
numYearsValidation <- 2
numYearsTraining1 <- 7
numYearsTraining <- numYearsTraining1 + numYearsValidation
...
atplist <- lapply(atplist, Filter_surface)
...
which_truthy <- function(x)
{
  return(table(which(isTruthy(x))))
}
...
clean.val <- function(validation_data, training_data)
{
  player.val <- unique(c(validation_data$winner_id, validation_data$loser_id))
  player.training <- unique(c(training_data$winner_id, training_data$loser_id))
  

  firstVal_ids <- player.val[which(!is.element(player.val, player.training))]
  print(length(firstVal_ids))
  return(firstVal_ids)
}
...
player.remove <- clean.val(atplist$atp2019, bind_rows(atplist[-1]))
player.remove2 <- clean.val(bind_rows(atplist[c(2,3)]) , bind_rows(atplist[-c(1,2,3)]))  # for validation


atplist <- lapply(atplist, remove_ids, vec = player.remove)
atplist <- lapply(atplist, remove_ids, vec = player.remove2)
```
...
calc_1stReturn_rally <- function(dataset, player_id)
{
  ...
  {
    t3 <- mean(temp2$l_1stRally_return)
  }
  else if(nrow(temp2) == 0) #if player never lost a match
  {
    t3 <- mean(temp1$w_1stRally_return)
  }
  else
  {
    t1 <- mean(temp1$w_1stRally_return)
    t2 <- mean(temp2$l_1stRally_return)
    t3 <- mean(c(t1,t2)) #percentage of points won on serve for player i
  }
...
calc_2ndReturn_rally <- function(dataset, player_id)
{
  ...
  {
    t3 <- mean(temp2$l_2ndRally_return)
  }
  else if(nrow(temp2) == 0) #if player never lost a match
  {
    t3 <- mean(temp1$w_2ndRally_return)
  }
  else
  {
    t1 <- mean(temp1$w_2ndRally_return)
    t2 <- mean(temp2$l_2ndRally_return)
    t3 <- mean(c(t1,t2)) #percentage of points won on serve for player i
  }
...
```{r}
remove_cols_3 <- function(x) # surface no longer removed here
{
  new <- x[,-which(names(x) %in% c( "tourney_name", "tourney_level", "tourney_date", "match_num", "winner_id", "loser_id", "winner_name","loser_name", "w_total_2nd", "l_total_2nd", "w_2ndLost", "l_2ndLost", "w_1stWon_prob", "l_1stWon_prob", "w_2ndWon_prob", "l_2ndWon_prob", "w_1stWon_givenIn_prob", "l_1stWon_givenIn_prob"))]
  return(new)
...
```{r}
val.temp <- bind_rows(atplist[c(2,3)])
train.temp <- bind_rows(atplist[-c(1,2,3)]) #training for validation


validation_data1 <- calc_testing_data(val.temp, train.temp) 
validation_data2 <- remove_cols_3(validation_data1)
set.seed(2028)
validation_data3 <- create_output(validation_data2)
validation_data4 <- remove_cols_4(validation_data3)
...
tourney_id <- testing_data4$tourney_id
testing_data5 <- cbind(tourney_id,(testing_data4[,-which(names(testing_data4) == "tourney_id")] * timeD_test))
...
#for validation data
timeD_val <- validation_data4$timeDiscount
#so we don't multiply who won and lost
p1_won.val <- validation_data4$p1_won
p2_won.val <- validation_data4$p2_won

#so we don't multiply rank, rank_points, and age
rank.val <- validation_data4$rank
rank_points.val <- validation_data4$rank_points
age.val <- validation_data4$age

tourney_id <- validation_data4$tourney_id
validation_data5 <- cbind(tourney_id,(validation_data4[,-which(names(validation_data4) == "tourney_id")] * timeD_val))

validation_data5$p1_won <- p1_won.val
validation_data5$p2_won <- p2_won.val
validation_data5$rank <- rank.val
validation_data5$rank_points <- rank_points.val
validation_data5$age <- age.val

validation_data6 <- validation_data5[,-which(names(validation_data5) == "timeDiscount")]


#similarly, for training data for validation
training_data1 <- bind_rows(features[-c(1,2,3)]) # training data for validation
...
tourney_id <- training_data1$tourney_id
training_data2 <- cbind(tourney_id, (training_data1[,-which(names(training_data1) == "tourney_id")] * timeD_training))
...
validation_data <- standardizer(validation_data6)
...
training_data <- training_data[,-which(names(training_data)=="tourney_id")]
testing_data <- testing_data[,-which(names(testing_data)=="tourney_id")]
validation_data <- validation_data[,-which(names(validation_data) == "tourney_id")]

write.csv(training_data,"./train_test_data/hard_training_small.csv", row.names = FALSE)
write.csv(testing_data,"./train_test_data/hard_testing.csv", row.names = FALSE)
write.csv(validation_data,"./train_test_data/hard_validation.csv", row.names = FALSE)
```